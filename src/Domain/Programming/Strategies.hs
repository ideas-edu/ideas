...
fromBinFoldlS = label "foldl" $ 
  progS [declPatS "fromBin" (rhsS (foldlS consS nilS) [])]
    where
...
fromBinRecurS = label "explicit recursion"  $ -- Add tupling model solution! Add an uncurryS to recognise tuples and arguments
  progS [declFunS [ funS (lhsS "fromBin" [patConS "[]"]) 
                         (rhsS (intS "0") [])
                  , funS (lhsS "fromBin" [patInfixConS (patS "x") ":" (patS "xs")])
                         (rhsS (opS "+" (Just (opS "*" (Just (varS "x"))
...
fromBinTuplingS = label "Tupling"  $ -- Add an uncurryS to recognise tuples and arguments
  progS [declFunS [funS (lhsS "fromBin" [patS "x"]) 
    (rhsS
      (varS "fromBin'" # [varS "x", opS "-" (Just (varS "length" # [varS "x"])) (Just (intS "1"))])
      [ declFunS [ funS (lhsS "fromBin'" [patConS "[]", patWildcardS]) (rhsS (intS "0") [])
                  , funS (lhsS "fromBin'" [patInfixConS (patS "y") ":" (patS "ys"), patS "z"]) 
                         (rhsS (opS "+" (Just (opS "*" (Just (varS "y"))
                                                       (Just (opS "^" (Just (intS "2")) 
                                                                      (Just (varS "z"))))))
...
stringToStrategy = either left getRules . compile
  where left err = error $ "Compile error: " ++ err

class Undefined a => GetRules a where
  getRules :: a -> Strategy a

instance GetRules Module where
  getRules (Module_Module _ _ _ body) = introModule <**> getRules body

instance GetRules Body where
  getRules (Body_Body _ _ decls) = 
    introBody (length decls) <**> mapSeqS getRules decls

instance GetRules MaybeDeclarations where
  getRules mdecls = 
    case mdecls of
      MaybeDeclarations_Nothing    -> toStrategy $ introMaybeDecls 0
      MaybeDeclarations_Just decls -> introMaybeDecls (length decls) <**>
                                      mapSeqS getRules decls

instance GetRules Declaration where
  getRules d = 
    case d of 
      Declaration_PatternBinding _ pattern rhs -> introDeclPatBinding <**> 
                                                  getRules pattern <**>
                                                  getRules rhs
      Declaration_FunctionBindings _ funbs -> introDeclFunBindings (length funbs) <**>
                                              mapSeqS getRules funbs
      _                                    -> error $ "No instance for: " ++ show d

instance GetRules MaybeExpression where
  getRules mexpr = 
    case mexpr of
      MaybeExpression_Nothing   -> toStrategy $ introMaybeExpr False
      MaybeExpression_Just expr -> introMaybeExpr True <**> getRules expr

instance GetRules Expression where
  getRules expr = 
    case expr of 
      Expression_NormalApplication _ fun args       -> introExprNormalApplication (length args) <**>
                                                       getRules fun <**> mapSeqS getRules args
      ...
      Expression_List              _ exprs          -> introExprList (length exprs) <**> 
                                                       mapSeqS getRules exprs
      Expression_Tuple             _ exprs          -> introExprTuple (length exprs) <**>
                                                       mapSeqS getRules exprs
      Expression_Let               _ decls expr     -> introExprLet (length decls) <**> mapSeqS getRules decls <**>
                                                       getRules expr
      Expression_Lambda            _ ps expr        -> introExprLambda (length ps) <**> mapSeqS getRules ps <**>
                                                       getRules expr
      ...
      RightHandSide_Guarded    _ gexprs mdecls -> 
          case mdecls of
            MaybeDeclarations_Just decls -> introRHSGuarded (length gexprs) (length decls) <**>
                                            mapSeqS getRules gexprs <**>
                                            mapSeqS getRules decls
            _                            -> introRHSGuarded (length gexprs) 0 <**>
                                            mapSeqS getRules gexprs

instance GetRules GuardedExpression where
  getRules (GuardedExpression_GuardedExpression _ guard expr) = 
    introGuardedExpr <**> getRules guard <**> getRules expr

instance GetRules FunctionBinding where
  getRules (FunctionBinding_FunctionBinding _ lhs rhs) = 
    introFunBinding <**> getRules lhs <**> getRules rhs
...
instance GetRules Pattern where
  getRules p = 
    case p of
      ...
      Pattern_Tuple            _ ps        -> introPatternTuple (length ps) <**> mapSeqS getRules ps
