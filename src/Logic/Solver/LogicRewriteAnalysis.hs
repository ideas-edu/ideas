...
module Logic.Solver.LogicRewriteAnalysis where
...
data Feedback = FB { ruleName         :: RuleName
                   ...
                   , removed          :: Logic {- or undefined -}
                   , introduced       :: Logic {- or undefined -} 
                   , correct          :: Logic {- or undefined -} 
                   } deriving Show
...
rewriteAnalyse :: Logic -> Logic -> Feedback
...
truefalse :: Logic -> Logic -> Maybe Feedback
...
eliminateImpl :: Logic -> Logic -> Maybe Feedback
...
eliminateEqv :: Logic -> Logic -> Maybe Feedback
...
deMorgan           :: Logic -> Logic -> Maybe Feedback
...
doubleNeg          :: Logic -> Logic -> Maybe Feedback
...
distributeAndOverOr :: Logic -> Logic -> Maybe Feedback
...
commutativity :: Logic -> Logic -> Maybe Feedback
...
{-------------------------------------------------------------------------------------
 ...
 calculates the difference between these formulae. Logic g is the
 ...
 - or the formula parts that are removed and/or introduced with
   respect to formula f (of type Maybe: Just(Logic, Logic). 

  This function must be expanded: what if T or F is added on top level(?).
--------------------------------------------------------------------------------------}


diff :: (Logic, Logic) -> Maybe (Logic, Logic)
...
allowedRuleApplied :: Logic -> Logic ->  RuleInfo
