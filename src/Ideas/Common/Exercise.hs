...
import Ideas.Common.Strategy hiding (fail, not, repeat, replicate)
...
makeExercise :: (Eq a, IsTerm a, Show a) => Exercise a
makeExercise =
  emptyExercise
    { prettyPrinter = show
    , similarity = (==)
...
ruleOrderingWith bs r1 r2 =
  let xs = map getId bs
  in  case (elemIndex (getId r1) xs, elemIndex (getId r2) xs) of
...
data Dynamic a
  = Dyn (forall b. Typeable b => Maybe b)
...
showThisDerivation
  :: Derivation (Rule b, Environment) (Context a) -> Exercise a -> String
...
diffEnvironment
  :: HasEnvironment a => Derivation s a -> Derivation (s, Environment) a
...
defaultDerivation
  :: Exercise a
  -> a
  -> Maybe (Derivation (Rule (Context a), Environment) (Context a))
defaultDerivation ex = listToMaybe . allDerivations ex

allDerivations
  :: Exercise a -> a -> [Derivation (Rule (Context a), Environment) (Context a)]
allDerivations ex =
